...
val CHANGELOG: List<ChangelogEntry> = listOf(
    ChangelogEntry(
        ...
        listOf(
            ...
            "교과교실 교실별 현황: 타임·건물·교실을 고르면 신청한 학생과 사유를 확인",
            "면학실·도서관 빠른 예약: 자주 쓰는 자리를 최대 4개 저장해 맨 위 버튼으로 바로 신청",
            "좌석 신청이 붐빌 때: 한 번만 보내고 응답을 기다리며 진행 표시, 결과는 새로 받은 좌석표로 확인",
            "위젯 급식 알레르기 경고를 빨간 글씨로 메뉴와 구분",
            "연강은 한 수업으로 보고 '다음 일정'에 겹쳐 뜨지 않게",
            "새로고침 때 학사시스템과 심야면학을 따로 확인 (한쪽이 실패해도 다른 쪽은 계속)",
            "다른 앱에 갔다 돌아올 때 가끔 뜨던 오류 창 수정",
        ),
    ),
    ChangelogEntry(
        "10.3",
    ...
    ChangelogEntry(
        ...
        listOf(
            ...
            "좌석에 신청한 학생 이름 표시 (남 파랑·여 분홍), 지정석 표시",
            "신청 기기 상태 표시와 '이 기기로 등록' 버튼 (면학실·도서관 각각)",
            "신청 내역을 앱 안 목록으로 (교과교실·도서관), 카드 순서 면학실→도서관→교과교실→외출·외박",
            "실시간 일정(Now Bar): 남은 시간이 절전 중 늦게 갱신되던 문제 — 정확 알람 + 시스템 카운트다운, 설치 후 권한 안내",
            "액체 유리: 헤더 섬·버튼 알약·팝업·선택 칩에 굴절 유리 적용, 하단 바 캡슐은 끌 때만 굴절, 무지개 테 제거, 배경 사진 반영",
            "다이얼로그를 화면 아래 시트로, 시트 자체만 흐림",
            "헤더 제목을 버튼 줄과 수평으로, 탭을 바꾸면 항상 맨 위에서 시작",
            "재설치 후 실행마다 꺼지던 문제, 비밀번호 키보드 추천 줄 노출, 급식 날짜 칩의 점 표시 정리",
        ),
    ),
    ChangelogEntry(
        "9.2",
        listOf(
            ...
            "앱을 지웠다 다시 설치하면 실행마다 꺼지던 문제 수정 (백업으로 되살아난 계정 파일을 못 열던 것)",
        ),
    ),
    ChangelogEntry(
        "9.1",
        listOf(
            ...
            "설정 → 실시간 일정: 알림이나 Now Bar 허용이 꺼져 있으면 켜는 방법을 안내하고 바로 설정으로 이동",
        ),
    ),
    ChangelogEntry(
        "9.0",
    ...
    ChangelogEntry(
        ...
        listOf(
            ...
            "아래로 당겨 새로고침 (모든 탭)",
            "3버튼 내비게이션에서 하단 바가 잘리던 문제 수정",
            "급식: 주간 보기에서 다른 날을 본 뒤 일간 보기로 돌아오면 오늘 식단으로",
            "위젯 강조 글자 색을 설정에서 선택 — 종류별 색 / 앱 강조 색 / 글자색과 같게",
            "위젯·홈의 '다음 일정'은 쉬는 시간을 건너뛰고 다음 수업·면학을 표시, 위젯 급식 창에도 다음 일정",
            "다음 달 중간·기말고사도 미리 찾아 홈 학사일정 카드에 D-day 표시",
            "설정 화면의 부연 설명 문구 정리, 설치 안내를 실제 Play 프로텍트 흐름으로",
        ),
    ),
    ChangelogEntry(
        "8.1",
